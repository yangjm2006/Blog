...
struct Point {
	...
	Point operator-(const Point& _) const { return Point(x - _.x, y - _.y); }
	bool operator<(const Point& _) const { return x < _.x || (x == _.x && y < _.y); }
	ll operator*(const Point& _) const { return x * _.x + y * _.y; }
	ll operator^(const Point& _) const { return x * _.y - y * _.x; }
	ll operator/(const Point& _) const { return x * _.y == y * _.x; }
};
typedef Point Vector;
int Convex_hull(Point* p, int n, Point* ch) {
...
### 带修莫队
```cpp
int n, m, len, Qn, Cn, a[N], ans[N];
struct Q {
	int l, r, id, t;
	bool operator<(const Q& x) const {
		if (l / len == x.l / len) {
			if (r / len == x.r / len) return t < x.t;
			return r < x.r;
		}
		return l < x.l;
	}
...
void change(int t) {
	if (c[t].pos >= L && c[t].pos <= R) {
		if (cnt[c[t].col] == 0) res++;
		cnt[c[t].col]++;
		cnt[a[c[t].pos]]--;
		if (cnt[a[c[t].pos]] == 0) res--;
	}
	swap(c[t].col, a[c[t].pos]);
}
void Mo() {
	len = pow(n, 2.0 / 3);
	sort(q + 1, q + 1 + Qn);
...
int main() {
	...
	for (int i = 1, l, r; i <= m; i++) {
		...
		cin >> ch >> l >> r;
		if (ch == 'Q')
			q[++Qn].l = l, q[Qn].r = r, q[Qn].id = Qn, q[Qn].t = Cn;
		else
			c[++Cn].pos = l, c[Cn].col = r;
	}
	Mo();
